...
import { RefreshCw, Plus, Trash2, AlertTriangle, CheckCircle } from "lucide-react";
...
export default function RenewalRadar() {
  ...
  const buckets = {
    "≤30 days": sorted.filter((a) => {
      const d = differenceInDays(parseISO(a.renewalDate), new Date());
      return d >= 0 && d <= 30;
    }),
    "31–60 days": sorted.filter((a) => {
      const d = differenceInDays(parseISO(a.renewalDate), new Date());
      return d > 30 && d <= 60;
    }),
    "61–90 days": sorted.filter((a) => {
      const d = differenceInDays(parseISO(a.renewalDate), new Date());
      return d > 60 && d <= 90;
    }),
    "90+ days": sorted.filter((a) => {
      const d = differenceInDays(parseISO(a.renewalDate), new Date());
      return d > 90;
    }),
  };
        ...
        <div className="mb-4 p-3 bg-[#0D1117] border border-blue-500/30 rounded-xl space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <input
              ...
              className="px-2.5 py-1.5 bg-[#161B22] border border-[#21262D] rounded-lg text-xs text-[#E6EDF3] placeholder-[#8B949E] focus:outline-none focus:border-blue-400"
            />
            <select
              value={form.tier}
              onChange={(e) => setForm({ ...form, tier: e.target.value })}
              className="px-2.5 py-1.5 bg-[#161B22] border border-[#21262D] rounded-lg text-xs text-[#E6EDF3] focus:outline-none focus:border-blue-400"
            >
          ...
          <div className="flex gap-2">
            {HEALTH_OPTIONS.map((h) => (
              <button
                key={h.value}
                onClick={() => setForm({ ...form, health: h.value as Account["health"] })}
                className={`px-2 py-1 rounded text-xs font-medium transition-colors ${form.health === h.value ? h.color : "text-[#8B949E] bg-[#161B22]"}`}
              >
                {h.label}
              </button>
            ))}
          ...
          <input
            ...
            className="w-full px-2.5 py-1.5 bg-[#161B22] border border-[#21262D] rounded-lg text-xs text-[#E6EDF3] placeholder-[#8B949E] focus:outline-none focus:border-blue-400"
          />
            ...
            <div key={label}>
              ...
              <div className="space-y-1">
                {accs.map((acc) => {
                  ...
                  const healthColor = acc.health === "green" ? "text-green-400" : acc.health === "yellow" ? "text-yellow-400" : "text-red-400";
                  return (
                    <div key={acc.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-[#21262D] group transition-colors">
                      <button onClick={() => toggleHealth(acc.id)} className={`w-2 h-2 rounded-full flex-shrink-0 ${acc.health === "green" ? "bg-green-400" : acc.health === "yellow" ? "bg-yellow-400" : "bg-red-400"}`} title="Click to cycle health status" />
                      ...
                      <div className="text-right flex-shrink-0">
                        {acc.arr && <div className="text-xs font-medium text-[#8DC647]">${acc.arr}</div>}
